...
export async function GET() {
  ...
  try {
    // Service client after auth — RLS on message_reads/messages may block user JWT reads.
    const service = await createServiceClient();
    const result = await fetchDriverUnreadMessage(service, user.id);
...
export async function POST(request: Request) {
  ...
  try {
    const service = await createServiceClient();
    await markDriverMessageRead(service, user.id, readId);
