-- Adds `need_cars` alert type (high taxi demand / "Bilar behövs").
-- Run in Supabase SQL Editor on existing projects.
-- Safe to re-run.

ALTER TYPE public.alert_type ADD VALUE IF NOT EXISTS 'need_cars';
...
AS $$
  SELECT CASE p_type
    ...
    WHEN 'need_cars' THEN interval '15 minutes'
